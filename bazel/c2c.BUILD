...
""" + select({
        "@brpc//bazel/config:brpc_with_sse42": """-e 's/ HAVE_SSE42/ HAVE_SSE42 1/' \
""",
        "//conditions:default": """-e 's/ HAVE_SSE42/ HAVE_SSE42 0/' \
""",
    }) + select({
        "@brpc//bazel/config:brpc_with_glog": """-e 's/ CRC32C_TESTS_BUILT_WITH_GLOG/ CRC32C_TESTS_BUILT_WITH_GLOG 1/' \
""",
...
cc_library(
    ...
    copts = select({
        "@brpc//bazel/config:brpc_with_sse42": ["-msse4.2"],
        "//conditions:default": [],
    }),
...
cc_test(
    ...
    deps = [
        ...
        "@brpc//bazel/config:brpc_with_glog": ["@com_github_google_glog//:glog"],
        "//conditions:default": [],
    }),
)